...
async function enrichWithTrestle(phone: string) {
  const key = process.env.TRESTLE_API_KEY
  if (!key) return null
  const cleaned = phone.replace(/\D/g, '')
  if (cleaned.length < 10) return null
  try {
    const res = await fetch(`https://api.trestleiq.com/3.2/phone?phone=${cleaned}`, { headers: { 'x-api-key': key } })
    if (!res.ok) return null
    const data = await res.json()
    if (!data.is_valid) return null
    const owner = data.owners?.[0] || null
    const addr = owner?.current_addresses?.[0] || null
    const emails: string[] = owner?.emails?.map((e: { email_address: string }) => e.email_address).filter(Boolean) || []
    return {
  ...
  } catch (e) { console.error('[Trestle error]', e); return null }
}

// ── ATTOM property enrichment (chained from Trestle address) ─────────────────
async function enrichWithAttom(address: string, city: string, state: string, zip: string) {
  ...
  try {
    ...
    const avmRes = await fetch(`https://api.gateway.attomdata.com/propertyapi/v1.0.0/avm/detail?address1=${a1}&address2=${a2}`, {
  ...
  } catch (e) { console.error('[Attom error]', e); return null }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (existingContact) {
      ...
      if (payload.phone) {
        enrichWithTrestle(payload.phone).then(async (trestle) => {
          if (!trestle) return
          await supabase.from('contacts').update(trestle).eq('id', contactId)
          console.log(`[Trestle] Enriched ${contactId}: ${trestle.trestle_owner_name} | ${trestle.trestle_line_type} | ${trestle.trestle_city}, ${trestle.trestle_state}`)
          // Chain Attom off Trestle address
          if (trestle.trestle_address && trestle.trestle_city && trestle.trestle_state) {
            enrichWithAttom(trestle.trestle_address, trestle.trestle_city, trestle.trestle_state, trestle.trestle_zip || '').then(async (attom) => {
              if (!attom) return
              await supabase.from('contacts').update(attom).eq('id', contactId)
              console.log(`[Attom] Enriched ${contactId}: AVM $${attom.attom_avm_value?.toLocaleString()} | ${attom.attom_sqft} sqft`)
